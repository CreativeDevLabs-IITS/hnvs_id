                ...
                <div class="d-flex flex-wrap gap-4 mt-5">
                    <div class="ps-4 pe-5 py-4 bg-white rounded-4 shadow" id="id_percentage" style="width: 300px;">
                        ...
                        <div class="fw-bold fs-1 mt-3 d-flex align-items-center gap-2">
                            <div id="circle-border" style="
                                width: 100px;
                                height: 100px;
                                border-radius: 50%;
                                background: conic-gradient( green 0deg, green odeg, #eee 0deg 360deg);
                                display: flex;
                                align-items: center;
...
<script>
    ...
    $(document).ready(() => {
        ...
        .then(data => {
            ...
            getPercentage(data.id-count, data.students);
        });
    ...
    function getPercentage(idCount, studentCount) {
        const circle = document.getElementById('circle-border');
        let percent = document.getElementById('percent');
        
        percent.textContent = 0 + '%';
        const color = '#3498DB';
        const percentage = Math.round(idCount / studentCount * 100);
        const degress = (percent / 100) * 360;
        circle.style.background = `conic-gradient(${color} 0deg ${degrees}deg, #eee ${degrees}deg 360deg)`;
